...
import type { FluxGymStatus, TrainingCheck, TrainingImage } from '../src/types';
...
export function prepareDataset(
  ...
  if (!root) throw new Error('FluxGym folder not found — set it in RP Settings → LoRA training.');
...
function comfyLorasDir(): string {
  ...
  if (!launch) {
    throw new Error(
      'Set your ComfyUI folder in RP Settings → Local images first, so WICKED knows where to install the LoRA.'
    );
  }
  for (const dir of [
...
export async function launch(): Promise<{ started: boolean; message: string }> {
  ...
  if (!root) {
    return {
      started: false,
      message: 'FluxGym not found. Install it with Pinokio, or set its folder in RP Settings.',
    };
  }
  const python =
