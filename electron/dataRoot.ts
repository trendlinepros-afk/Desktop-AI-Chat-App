...
import type { DataLocations } from '../src/types';
...
//   <root>/RP Memory/          — the role-play vault (rpVaultPath)
...
export function getLocations(): DataLocations {
  ...
  return {
    ...
    dbPath: path.join(app.getPath('userData'), 'wicked.db'),
    vaultPath: s.vaultPath,
    rpVaultPath: s.rpVaultPath,
...
export async function consolidate(root: string): Promise<string[]> {
  ...
  moveFolderSetting(
    s.rpVaultPath,
    'RP Memory',
    (p) => db.saveSettings({ rpVaultPath: p }),
    'Role-play memory vault'
  );
