...
import type { RPMessage, RPPerson, RPPersona, RPScene } from '../types';
...
// Sentinel returned by generateFor when a character chooses to stay silent.
const PASS = '\u0000PASS\u0000';
const FORCE_NUDGE =
  '(This message is directed at you — respond in character now; do not stay silent.)';

// Did the model decline to speak this turn?
...
interface RPState {
  personas: RPPersona[];
  persons: RPPerson[]; // visual identities (LoRA + trigger + preset) personas can select
  scenes: RPScene[];
  ...
  summarizing: boolean;
  suggesting: boolean; // generating a suggested reply for you
  ...
  updatePerson: (
    ...
    patch: Partial<
      Pick<
        RPPerson,
        'name' | 'triggerWord' | 'imagePrompt' | 'loraName' | 'loraStrength' | 'status' | 'datasetSlug' | 'previewImage'
      >
    >
  ) => Promise<void>;
  deletePerson: (id: string) => Promise<void>;
  personById: (id: string | null | undefined) => RPPerson | undefined;
  ...
  setRating: (id: string, rating: 'up' | 'down' | '') => Promise<void>;
  suggestReply: () => Promise<string>;
...
export const useRPStore = create<RPState>((set, get) => ({
  ...
  personById: (id) => (id ? get().persons.find((p) => p.id === id) : undefined),

  loadPersonas: async () => set({ personas: await window.polyglot.rpGetPersonas() }),
  loadPersons: async () => set({ persons: await window.polyglot.rpGetPersons() }),
  ...
  createPerson: async (data) => {
    const person = await window.polyglot.rpCreatePerson(data);
    await get().loadPersons();
    return person;
  },

  updatePerson: async (id, patch) => {
    await window.polyglot.rpUpdatePerson(id, patch);
    await get().loadPersons();
  },

  deletePerson: async (id) => {
    await window.polyglot.rpDeletePerson(id);
    // Personas that pointed at it were reset in the DB — refresh both lists.
    await Promise.all([get().loadPersons(), get().loadPersonas()]);
  },
  ...
  setRating: async (id, rating) => {
    await window.polyglot.rpSetMessageRating(id, rating);
    set({ messages: get().messages.map((m) => (m.id === id ? { ...m, rating } : m)) });
    const toast = useUIStore.getState().toast;
    if (rating === 'up') toast('👍 Marked as good — more replies like this', 'success');
    else if (rating === 'down') toast('👎 Marked as bad — fewer replies like this', 'info');
    else toast('Rating cleared', 'info');
  },

  // Draft a reply for YOUR own ("me") persona from the conversation so far, so you
  // can review/edit it before sending. Returns the text; never auto-sends.
  suggestReply: async () => {
    const sceneId = get().activeSceneId;
    const me = get().mePersona();
    const settings = useSettingsStore.getState().settings;
    ...
    if (!sceneId) return '';
    if (!me) {
      toast('Mark a persona as “me” first (checkbox in its editor)', 'error');
      return '';
    }
    ...
    try {
      const system =
        `You are "${me.name}", the user's own character in a role-play.` +
        (me.description.trim() ? `\nAbout you: ${me.description.trim()}` : '') +
        `\n\nWrite ${me.name}'s next message to continue the conversation naturally, in the first ` +
        `person, in an immersive style: brief *asterisk* narration of actions/feelings plus spoken ` +
        `dialogue. Advance the scene; do not repeat earlier lines or re-describe the setting. Reply ` +
        `with ONLY ${me.name}'s message — no quotes, no preface.`;
      const turns: RPTurn[] = [{ role: 'system', content: system }];
      for (const m of get().messages.slice(-MAX_HISTORY)) {
        if (m.kind === 'director') {
          turns.push({ role: 'user', content: `[DIRECTOR]: ${m.content}` });
        } else if (m.senderPersonaId === me.id) {
          turns.push({ role: 'assistant', content: m.content });
        } else {
          turns.push({ role: 'user', content: `${speakerName(get, m)}: ${m.content}` });
        }
      }
      return await grokComplete(settings.grokApiKey, me.model || settings.grokModel, turns, {
        temperature: 0.9,
        presencePenalty: 0.5,
...
// The driver for automatic replies. Characters only speak when it actually fits:
// the one you named must respond; otherwise each enabled character gets a chance
// but may stay silent ([silence]). A reply only pulls in another character if it
// addresses them by name — no forced round-robin — so they don't all talk every
// turn. Bounded by the safety limit; stops if someone asks YOU a question.
async function runAuto(get: Get, set: Set, sceneId: string, userText: string): Promise<void> {
  ...
  const addressed = findAddressed(userText, aiMembers);
  // If you named someone, only they answer (forced). Otherwise every enabled
  // character gets one chance to react, free to stay silent if it's not for them.
  const queue: { persona: RPPersona; force: boolean }[] = addressed
    ? [{ persona: addressed, force: true }]
    : aiMembers.map((persona) => ({ persona, force: false }));

  let replies = 0;
  let lastSpeakerId: string | null = null;
  while (queue.length > 0 && replies < limit && !cancelled && get().activeSceneId === sceneId) {
    const { persona, force } = queue.shift()!;
    if (persona.id === lastSpeakerId) continue; // never twice in a row
    const text = await generateFor(get, set, sceneId, persona, force ? FORCE_NUDGE : undefined, {
      allowPass: !force,
    });
    if (text === PASS) continue; // chose to stay silent — doesn't count
    replies++;
    lastSpeakerId = persona.id;
    const others = aiMembers.filter((p) => p.id !== persona.id);
    if (addressedToUser(text, me, others)) break; // they asked you — wait for you
    // Only continue if this reply explicitly addresses another character.
    const target = findAddressed(text, others);
    if (target) queue.push({ persona: target, force: true });
  }
...
async function generateFor(
  ...
  persona: RPPersona,
  nudge?: string,
  opts?: { history?: RPMessage[]; replaceId?: string; allowPass?: boolean }
): Promise<string> {
  const settings = useSettingsStore.getState().settings;
  const toast = useUIStore.getState().toast;
  ...
  if (opts?.allowPass && others.length > 0) {
    system +=
      `\n\nYou do NOT have to speak every turn. If this latest message wasn't directed at ` +
      `${persona.name} and ${persona.name} has nothing meaningful or in-character to add right ` +
      `now, reply with EXACTLY "[silence]" and nothing else. Staying quiet is natural — only ` +
      `speak when it genuinely fits ${persona.name} to react.`;
  }
  if (me) {
  ...
  // Steer toward replies the user thumbs-upped and away from thumbs-downed ones.
  const rated = get().messages.filter(
    (m) => m.senderPersonaId === persona.id && (m.rating === 'up' || m.rating === 'down')
  );
  const disliked = rated.filter((m) => m.rating === 'down').slice(-2);
  const liked = rated.filter((m) => m.rating === 'up').slice(-2);
  if (disliked.length || liked.length) {
    system += `\n\nThe reader has rated some of your past replies:`;
    for (const m of disliked) {
      system += `\n- DISLIKED — avoid this style/content: "${m.content.slice(0, 160)}"`;
    }
    for (const m of liked) {
      system += `\n- LIKED — write more like this: "${m.content.slice(0, 160)}"`;
    }
  }
  ...
  // The character chose to stay silent — don't post anything.
  if (opts?.allowPass && isPass(text)) {
    if (get().activeSceneId === sceneId) set({ speakingId: null });
    return PASS;
  }
