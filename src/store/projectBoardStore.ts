  ...
  return {
    ...
    load: async () => {
      const [projects, dataFolder] = await Promise.all([
        window.polyglot.pbGetProjects(),
        window.polyglot.pbGetDataFolder(),
      ]);
    ...
    createProject: async (name, icon) => {
      const project = await window.polyglot.pbCreateProject(name, icon);
      set({ projects: [...get().projects, project] });
      await get().selectProject(project.id);
    },

    renameProject: async (id, name) => {
      await window.polyglot.pbRenameProject(id, name);
    ...
    deleteProject: async (id) => {
      ...
      await window.polyglot.pbDeleteProject(id);
    ...
    selectProject: async (id) => {
      ...
      const board = await window.polyglot.pbLoadBoard(id);
    ...
    flushSave: async () => {
      ...
      await window.polyglot.pbSaveBoard(activeProjectId, { ...board, updatedAt: Date.now() });
    },

    changeDataFolder: async (folder, migrate) => {
      await get().flushSave();
      await window.polyglot.pbSetDataFolder(folder, migrate);
      set({ activeProjectId: null, board: null });
      await get().load();
    },
  };
