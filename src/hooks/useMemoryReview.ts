  ...
  const saveReview = useCallback(
    async (
      ...
      for (const idea of review.ideas) {
        ...
        const p = await window.polyglot.vaultWriteNote('Ideas', `idea-${idea.slice(0, 40)}`, ideaMd);
        ideaPaths.push(p);
      }

      await window.polyglot.vaultRegenerateIndex();

      // Generate + store an embedding for the new note when possible.
      if (settings.openaiApiKey && settings.semanticIndexingEnabled) {
        try {
          const embedding = await embedText(settings.openaiApiKey, review.summary + '\n' + noteMd);
          await window.polyglot.vaultSaveEmbedding(notePath, embedding);
        } catch (err) {
