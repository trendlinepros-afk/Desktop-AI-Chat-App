...
// read-aloud). STT and TTS run against the OpenAI API in the renderer, so the
// same code works in the desktop app and the LAN web portal.
//
...
function client(settings: Settings): OpenAI {
  if (!settings.openaiApiKey) {
    throw new Error('Voice needs an OpenAI API key — add one in Settings.');
  }
  return new OpenAI({ apiKey: settings.openaiApiKey, dangerouslyAllowBrowser: true });
}
...
export async function transcribe(blob: Blob, mime: string, settings: Settings): Promise<string> {
  const baseMime = (mime || blob.type || 'audio/webm').split(';')[0];
  const ext = baseMime.split('/')[1] || 'webm';
  const file = new File([blob], `speech.${ext}`, { type: baseMime });
  const res = await withRetry(
    () => client(settings).audio.transcriptions.create({ file, model: settings.sttModel }),
    { retries: 1 }
  );
...
class TtsQueue {
  ...
  private async pump(): Promise<void> {
    if (this.fetching || this.fetched || this.pending.length === 0 || !this.settings) {
      this.settle();
      return;
    }
