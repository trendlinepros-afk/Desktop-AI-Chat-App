...
export async function exportChat(chat: Chat, format: 'markdown' | 'pdf'): Promise<string | null> {
  const messages = await window.polyglot.getMessages(chat.id);
  const safeName = chat.title.replace(/[^a-z0-9-_ ]/gi, '').trim() || 'chat';
  if (format === 'markdown') {
    return window.polyglot.exportMarkdown(safeName, chatToMarkdown(chat, messages));
  }
  return window.polyglot.exportPDF(safeName, chatToHtml(chat, messages));
}
