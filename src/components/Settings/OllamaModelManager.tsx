...
export function OllamaModelManager({
  ...
  const row = (m: CatalogModel) => {
    ...
    return (
      <div key={m.id} className="rounded-lg border border-edge bg-surface p-3">
        <div className="flex items-center gap-2">
          <span className="font-medium text-text-primary">{m.label}</span>
          <span className="text-xs text-text-muted">{m.size}</span>
          <div className="flex-1" />
          {installedNow ? (
            <>
              <span className="text-xs text-idea">✓ Installed</span>
              {onSelect && (
                <button
        ...
        <p className="mt-1 text-xs text-text-muted">{m.goodAt}</p>
        {prog && (
