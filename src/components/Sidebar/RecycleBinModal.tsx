...
export function RecycleBinModal({ onClose }: { onClose: () => void }) {
  ...
  const refresh = () => window.polyglot.getDeletedChats().then(setItems);
  useEffect(() => {
    refresh();
  }, []);
  ...
  const restore = async (id: string) => {
    await window.polyglot.restoreChat(id);
    await loadChats();
    refresh();
  ...
  const purge = async (id: string) => {
    await window.polyglot.purgeChat(id);
    refresh();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[80vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-edge bg-topbar shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {items.length === 0 ? (
            <div className="py-8 text-center text-sm text-text-muted">The recycle bin is empty.</div>
          ) : (
              ...
              {items.map((c) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-text-muted">{daysLeft(c.deletedAt)} days left</div>
                  </div>
                  <button
