  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-edge bg-surface px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-xs text-text-muted">{persona.vaultPath || 'No folder set'}</div>
      </div>
      <button
        onClick={startChat}
      ...
      <button onClick={onEdit} className="rounded-md px-2 py-1 text-xs text-text-muted hover:text-text-primary">
        Edit
      </button>
      <button
        onClick={() => {
...
function PersonaEditor({
  ...
  const pickFolder = async () => {
    const p = await window.polyglot.openVaultFolderDialog();
    if (p) setVaultPath(p);
  };
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    try {
      const { fileCount, sample } = await window.polyglot.brainFolderDigest(vaultPath);
      if (fileCount === 0) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[90vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-edge bg-topbar shadow-2xl">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <div>
            <label className="mb-1 block text-xs text-text-muted">Brain folder (Obsidian)</label>
