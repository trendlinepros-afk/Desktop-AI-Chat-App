...
export function UpdateChecker() {
  ...
  return (
    <div className="flex items-center justify-center gap-3 border-t border-white/5 bg-app px-4 py-2 text-xs text-text-muted">
      <span>Polyglot v{version || '…'}</span>
      <button
        onClick={check}
        disabled={checking}
        className="rounded-md border border-white/10 px-2.5 py-1 text-text-muted transition hover:text-text-primary disabled:opacity-50"
      >
