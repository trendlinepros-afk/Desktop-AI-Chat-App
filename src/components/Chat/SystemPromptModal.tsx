...
export function SystemPromptModal({ chat, onClose }: { chat: Chat; onClose: () => void }) {
  ...
  const loadTemplates = () => window.polyglot.getTemplates().then(setTemplates);
  useEffect(() => {
    loadTemplates();
  }, []);
  ...
  const saveAsTemplate = async () => {
    ...
    await window.polyglot.saveTemplate(name, value);
    setTemplateName('');
    loadTemplates();
      ...
      <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-edge bg-topbar shadow-2xl">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          <textarea
            ...
            className="w-full rounded-lg border border-edge bg-surface px-3 py-2 text-sm text-text-primary outline-none focus:border-accent"
          />
          ...
          <div>
            ...
            {templates.length === 0 ? (
              <div className="text-xs text-text-muted/70">No saved templates yet.</div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {templates.map((t) => (
                  ...
                  >
                    ...
                    <button
                      onClick={async () => {
                        await window.polyglot.deleteTemplate(t.id);
                        loadTemplates();
                      }}
            ...
            <div className="mt-2 flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-edge bg-surface px-2 py-1.5 text-sm outline-none focus:border-accent"
              />
              <button
        ...
        <div className="flex items-center justify-end gap-2 border-t border-edge px-5 py-3">
          <button
            onClick={() => setValue('')}
            className="mr-auto rounded-lg border border-edge px-3 py-2 text-sm text-text-muted hover:text-text-primary"
          >
            Clear
          </button>
          <button
