...
export function BoardCanvas({ projectId, board }: { projectId: string; board: BoardData }) {
  ...
  const importImage = async () => {
    const res = await window.polyglot.pbImportImage(projectId);
    if (!res) return;
    ...
    const onPaste = (e: ClipboardEvent) => {
      ...
      if (image) {
        ...
        reader.onload = async () => {
          ...
          const { assetId } = await window.polyglot.pbSaveAsset(projectId, dataUrl);
          const c = viewportCenter();
  ...
  const toolButton = (t: Tool, label: string) => (
    <button
      ...
      className={`rounded-lg px-2.5 py-1.5 text-sm ${
        tool === t ? 'bg-accent/20 text-accent' : 'text-text-muted hover:bg-hover hover:text-text-primary'
      }`}
    >
      {label}
    </button>
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-1.5 border-b border-edge bg-topbar px-3 py-2">
        ...
        <button
          onClick={importImage}
          className="rounded-lg px-2.5 py-1.5 text-sm text-text-muted hover:bg-hover hover:text-text-primary"
        >
          🖼 Add image
        </button>
        {tool === 'draw' && (
            ...
            {PEN_SIZES.map((s) => (
              <button
                ...
                className={`flex h-7 w-7 items-center justify-center rounded-lg ${
                  penSize === s ? 'bg-accent/20' : 'hover:bg-hover'
                }`}
              >
                <span
        ...
        <span className="ml-auto hidden text-xs text-text-muted xl:block">
      ...
      <div ref={scrollRef} className="relative flex-1 overflow-auto">
        ...
        >
          ...
          {selectedItem && !editingId && tool === 'select' && (
            <div
              className="absolute flex items-center gap-1.5 rounded-lg border border-edge bg-topbar p-1 shadow-lg"
            ...
            >
              <select
                ...
                className="rounded border border-edge bg-surface px-1 py-1 text-xs outline-none"
              >
                {PRIORITIES.map((p) => (
              ...
              <input
                ...
                className="w-28 rounded border border-edge bg-surface px-1.5 py-1 text-xs outline-none"
              />
              ...
              <button
                ...
                className="rounded px-1.5 py-0.5 text-xs hover:bg-hover"
              >
                ⬆️
              </button>
              <button
