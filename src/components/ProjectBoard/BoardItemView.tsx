...
function useAsset(projectId: string, assetId?: string): string | null {
  ...
  useEffect(() => {
    ...
    let alive = true;
    window.polyglot.pbGetAsset(projectId, assetId).then((u) => {
...
export function BoardItemView({
  ...
  return (
    <div
      ...
      } ${item.type === 'text' ? 'bg-surface' : 'bg-surface/40'}`}
      style={{
        left: item.x,
        top: item.y,
        width: item.w,
    ...
    >
      ...
      {item.type === 'text' ? (
        editing ? (
          ...
          <div className="h-full w-full overflow-hidden whitespace-pre-wrap p-3 text-sm">
            {item.text || (
              <span className="text-text-muted">Empty note — double-click to edit</span>
            )}
          </div>
        )
      ) : url ? (
      ...
      {selected && !editing && (
        <div
          ...
          className="absolute -bottom-1.5 -right-1.5 h-4 w-4 cursor-nwse-resize rounded-sm border border-accent bg-app"
        />
      )}
    </div>
  );
