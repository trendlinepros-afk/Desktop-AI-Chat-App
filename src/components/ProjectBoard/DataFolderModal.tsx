...
export function DataFolderModal({ onClose }: { onClose: () => void }) {
  ...
  const choose = async () => {
    const folder = await window.polyglot.pbChooseDataFolder();
  ...
  return (
    <div data-pb-modal className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-edge bg-topbar shadow-2xl">
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <div>
            ...
            <div className="break-all rounded-lg border border-edge bg-surface px-3 py-2 font-mono text-xs">
              {dataFolder || '…'}
            </div>
          </div>
