...
export function LinkedChatsPanel({ chat, onClose }: { chat: Chat; onClose: () => void }) {
  ...
  useEffect(() => {
    window.polyglot.getChatLinks(chat.id).then((ids) => setLinked(new Set(ids)));
  }, [chat.id]);
  ...
  const toggle = async (id: string) => {
    ...
    if (next.has(id)) {
      next.delete(id);
      await window.polyglot.removeChatLink(chat.id, id);
    } else {
      next.add(id);
      await window.polyglot.addChatLink(chat.id, id);
    }
    setLinked(next);
  };
    ...
    <>
      ...
      <div className="absolute right-0 z-40 mt-2 w-72 rounded-xl border border-edge bg-topbar p-2 shadow-2xl">
        ...
        <div className="max-h-64 overflow-y-auto">
          {candidates.length === 0 ? (
            <div className="px-2 py-3 text-center text-xs text-text-muted">No chats</div>
          ) : (
            candidates.map((c) => (
              <label
                key={c.id}
