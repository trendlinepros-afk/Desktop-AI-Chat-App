...
export function BuildPromptModal({ chat, onClose }: { chat: Chat; onClose: () => void }) {
  ...
  const saveToBrain = async () => {
    ...
    try {
      ...
      const path = await window.polyglot.vaultWriteNote(
        'Projects',
        `${title} build prompt`,
        planNoteMarkdown(`${title} — Build Prompt`, prompt)
      );
      await window.polyglot.vaultRegenerateIndex();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-edge bg-topbar shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {!loading && !error && (
            <textarea
              ...
              className="h-[55vh] w-full resize-none rounded-lg border border-edge bg-surface px-3 py-2 font-mono text-xs text-text-primary outline-none focus:border-accent"
            />
          )}
        </div>
